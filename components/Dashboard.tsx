    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 pb-20">
      <header className="mb-10 relative">
        ...
        <div className="flex justify-between items-center mb-10 relative z-10">
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <div className={`inline-flex items-center gap-1.5 bg-white shadow-sm px-3 py-1 rounded-full border transition-colors duration-500 ${showOffline ? 'border-rose-100' : 'border-slate-100'}`}>
        ...
        {waiterCalls.length > 0 && (
          <div className="mb-8 animate-in slide-in-from-top fade-in duration-500">
             <div className="flex items-center justify-between mb-3 px-2">
                <div className="flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-orange-500 animate-ping"></div>
                   <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-900">Active Service Requests</h3>
                </div>
                <span className="text-[10px] font-black text-slate-400">{waiterCalls.length} Pending</span>
             </div>
                ...
                {waiterCalls.map(call => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <p className="text-[10px] font-black uppercase tracking-tight text-slate-400">{getTimeAgo(call.timestamp)}</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          <div className="bg-slate-950 rounded-[2.5rem] p-6 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between">
                ...
                <div className="flex items-center gap-5 relative z-10">
                   <div className="relative w-20 h-20 flex items-center justify-center shrink-0">
                      <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                         <circle cx="50" cy="50" r={radius} stroke="currentColor" strokeWidth="10" fill="transparent" className="text-white/5" />
                         <circle cx="50" cy="50" r={radius} stroke="currentColor" strokeWidth="10" fill="transparent" strokeDasharray={circumference} style={{ strokeDashoffset, transition: 'stroke-dashoffset 1.5s cubic-bezier(0.4, 0, 0.2, 1)' }} strokeLinecap="round" className="text-indigo-500 drop-shadow-[0_0_12px_rgba(99,102,241,0.6)]" />
                      </svg>
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-[2.5rem] p-6 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between">
             ...
             <div className="relative z-10">
                ...
                <div className="mt-3 flex items-center gap-2 opacity-90">
                  ...
                  <p className="text-[11px] font-bold uppercase tracking-tight">{activeOrdersCount} Open Orders</p>
                </div>
             </div>
        ...
        <div className="space-y-5">
           ...
           <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar scroll-smooth">
            <button onClick={() => setFilter('all')} className={`flex items-center gap-3 px-8 py-4 rounded-2xl border-2 transition-all whitespace-nowrap active:scale-95 ${filter === 'all' ? 'bg-slate-950 text-white border-slate-950 shadow-2xl shadow-slate-200 font-bold' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-300'}`}>
            ...
            <button onClick={() => setFilter('occupied')} className={`flex items-center gap-3 px-8 py-4 rounded-2xl border-2 transition-all whitespace-nowrap active:scale-95 ${filter === 'occupied' ? 'bg-orange-500 text-white border-orange-500 shadow-2xl shadow-orange-100 font-bold' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-300'}`}>
            ...
            <button onClick={() => setFilter('inactive')} className={`flex items-center gap-3 px-8 py-4 rounded-2xl border-2 transition-all whitespace-nowrap active:scale-95 ${filter === 'inactive' ? 'bg-emerald-600 text-white border-emerald-600 shadow-2xl shadow-emerald-100 font-bold' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-300'}`}>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
        {filteredTables.length > 0 ? (
          filteredTables.map((table) => (
            ...
            >
              {table.status === 'occupied' && <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>}
          ...
          <div className="col-span-full py-24 text-center">
            <div className="inline-flex items-center justify-center w-24 h-24 bg-slate-100 rounded-[2.5rem] mb-8 shadow-inner">
              <svg className="w-12 h-12 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 002-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
            </div>
